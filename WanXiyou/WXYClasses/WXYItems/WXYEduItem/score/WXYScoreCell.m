...
#import "WXYScoreCell.h"

@interface WXYScoreCell ()

@property(nonatomic, strong) WXYScoreDetailView *detailView;
...
@implementation WXYScoreCell

+ (instancetype)cellWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath {
    static NSString *identifier = @"teamList";
    WXYScoreCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[WXYScoreCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier indexPath:indexPath];
    }
    return cell;
}
...
- (WXYScoreDetailView *)detailView {
    if (_detailView == nil) {
        _detailView = [[WXYScoreDetailView alloc]initWithFrame:self.frame indexPath:self.indexPath.row];
    }
    return _detailView;
}
...
- (void)selectToShowDetailWithContentOffsetY:(CGFloat)contentOffsetY {
    ...
    NSLog(@"detailView.indexPath -- > %ld",self.detailView.indexPath);
